...
if frames_collected > 0:
    ...
    print(f"\nâœ… SUCCESS! {frames_collected} frames of {EXERCISE_NAME} have been successfully injected into custom_workout_dataset.csv!")
else:
...
try:
    ...
    print("ðŸ§  [SUCCESS] Machine Learning Workout Brain Loaded!")
except FileNotFoundError:
...
def load_registered_faces():
    ...
    print("\n" + "="*50)
    print("ðŸš€ [CAPSTONE READY] LONG-RANGE AI ENGINE...")
...
try:
    ...
    print("ðŸ’¾ Model successfully saved as 'workout_model.pkl'")
    print("You are now ready to plug this brain into gesture_engine.py!")

except FileNotFoundError:
    print("âŒ ERROR: Could not find 'custom_workout_dataset.csv'. Did you run collect_data.py first?")
except Exception as e:
    print(f"âŒ ERROR: {e}")
