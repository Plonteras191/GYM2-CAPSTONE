...
import { FiMaximize, FiCamera, FiWifiOff, FiLoader, FiUserCheck, FiArrowRight, FiRefreshCw } from 'react-icons/fi';
...
export default function SecurityMonitor() {
  ...
  // Connection states: 'RECONNECTING', 'LIVE', 'DISCONNECTED'
  ...
  const handleSyncAI = async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert("✅ AI Memory successfully synced with the Database!");
      } else {
  ...
  const [cameraConfig] = useState({
    ...
    streamUrl: `http://127.0.0.1:5000/security_feed` 
  });

  useEffect(() => {
    // Start the continuous stream automatically
    ...
    const logInterval = setInterval(async () => {
      try {
        ...
        console.error("Checking for new logs...");
      }
    }, 1000);
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto h-full flex flex-col">
      ...
      {/* 24/7 Operation: Manual toggle buttons removed */}
      <div className="flex flex-col lg:flex-row gap-6 flex-1 h-full min-h-0">
        ...
        <div ref={containerRef} className="w-full lg:w-3/4 bg-black rounded-2xl overflow-hidden relative border border-slate-300 dark:border-slate-700/50 shadow-xl flex flex-col group aspect-video lg:aspect-auto lg:h-full flex-shrink-0">
          ...
          <div className="flex-1 flex items-center justify-center relative bg-[#0a0a0c] overflow-hidden w-full h-full">
              {/* The image tag acts as the ping: onLoad means we have frames, onError means it broke */}
              <img 
                src={activeStreamUrl} 
                alt="Live CCTV" 
                ...
                onError={() => {
                  ...
                  // Self-Healing Loop: Wait 5 seconds, then try to hit the feed again if the computer has internet
                  setTimeout(() => {
                    if (navigator.onLine) {
                      setConnectionState('RECONNECTING');
                      setActiveStreamUrl(`${cameraConfig.streamUrl}?t=${new Date().getTime()}`);
